# Graph Report - sparingV1.1  (2026-08-16)

## Corpus Check
- 160 files · ~137,742 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2063 nodes · 2755 edges · 163 communities (149 shown, 14 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 364 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e457063a`
...
2. `User` - 37 edges
3. `Alert` - 27 edges
4. `scan_logger_liveness()` - 19 edges
5. `SPARING Frontend - Implementation Guide` - 19 edges
6. `generate_report()` - 18 edges
7. `SensorData` - 18 edges
8. `Base` - 17 edges
9. `LoggerStatus` - 17 edges
10. `Feature 1: Alert & Notification System — Implementation Plan` - 17 edges
...
Cohesion: 0.06
Nodes (65): last_record(), list_data(), AsyncSession, datetime, SensorData, SensorHealth, Alternating small noise around 7.0 — a realistic stable pH series., _series() (+57 more)

### Community 1 - "Database Models"
Cohesion: 0.14
Nodes (18): change_password(), login(), logout(), me(), AsyncSession, Change own password (requires current password verification)., Authenticate user and return JWT tokens., Logout user by blacklisting the current access token. (+10 more)
...
Cohesion: 0.10
Nodes (27): ingest_bulk(), ingest_state(), AsyncSession, Request, _validate_ranges(), DataOut, IngestBulkIn, IngestStateIn (+19 more)
...
Cohesion: 0.09
Nodes (26): JsonFormatter, BaseHTTPMiddleware, Request, RateLimitMiddleware, _make_site(), Only some params negative => ordinary impossible-value handling, no op_status., post_data fires trigger_alerts/detect_realtime via asyncio.create_task;     the, All water params carrying the same negative sentinel = an operational-status (+18 more)
...
Cohesion: 0.05
Nodes (24): @/Utils/analysis.js, barOptions, barSeries, chartData, colors, completeness, completenessPct, complianceMonth (+16 more)

### Community 14 - "Users Page"
Cohesion: 0.07
Nodes (27): accessMatrix, allSites, closeModal(), closeSitesModal(), { confirm }, deleteUser(), editingUser, filteredUsers (+19 more)
...
Cohesion: 0.07
Nodes (57): add_maintenance_log(), create_device(), delete_device(), delete_maintenance_log(), _device_data_filter(), get_device(), get_device_health(), list_devices() (+49 more)
...
Cohesion: 0.10
Nodes (27): get_key(), _global_secret(), _num(), post_data(), AsyncSession, Request, Extract a numeric field by any of `keys` (first non-None wins).      Returns (, Return the operational-status code when EVERY present water parameter     carri (+19 more)

### Community 22 - "Header Component"
Cohesion: 0.14
Nodes (32): create_alert_rule(), delete_alert_rule(), _get_site_by_uid(), list_alert_rules(), AsyncSession, update_alert_rule(), completeness(), compliance() (+24 more)
...
Cohesion: 0.08
Nodes (17): alerts, ANOMALY_LABELS, FIELD_LABELS, filters, { getAlerts, getSites, followupAlert, resolveAlert }, { isOperator }, loading, modal (+9 more)
...
### Community 85 - "Base"
Cohesion: 0.33
Nodes (7): DeclarativeBase, Base, ApiKey, AuthTokenBlacklist, IngestLog, SensorType, ViewerSite

### Community 86 - "Alert"
Cohesion: 0.09
Nodes (47): acknowledge_alert(), _build_alert_out(), followup_alert(), get_alert_count(), list_alerts(), AsyncSession, datetime, Close an alert. A follow-up note is mandatory (SOP: no closure without a record) (+39 more)
...
### Community 131 - "test_alerts_api.py"
Cohesion: 0.36
...
### Community 138 - "test_data_api.py"
Cohesion: 0.58
...
### Community 142 - "test_auth_api.py"
Cohesion: 0.43
...
### Community 149 - "load"
Cohesion: 0.40
Nodes (5): applyFilters(), changePage(), closeModal(), handleSubmit(), load()
...
## Knowledge Gaps
- **945 isolated node(s):** `name`, `version`, `description`, `type`, `dev` (+940 more)
...
- **Why does `Site` connect `Devices & Maintenance API` to `Anomaly Detection Engine`, `Database Models`, `test_alerts_api.py`, `Alert Engine & Rules`, `Sites & Data API`, `Report Generation`, `User`, `Ingestion & Time Utils`, `test_data_api.py`, `Metrics & Caching`, `Device Ingest Endpoint`, `Alert`, `Header Component`, `Base`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `User` connect `User` to `Database Models`, `test_alerts_api.py`, `Sites & Data API`, `test_data_api.py`, `get_current_user`, `test_auth_api.py`, `Devices & Maintenance API`, `Base`, `Alert`, `Header Component`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `dependencies` connect `Frontend Dependencies` to `package.json`, `xlsx`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Are the 45 inferred relationships involving `Site` (e.g. with `assign_viewer()` and `list_viewer_sites()`) actually correct?**
  _`Site` has 45 INFERRED edges - model-reasoned connections that need verification._
- **Are the 23 inferred relationships involving `User` (e.g. with `assign_viewer()` and `create_user()`) actually correct?**
...
- **Are the 14 inferred relationships involving `scan_logger_liveness()` (e.g. with `_check_logger_liveness()` and `test_brief_sensor_failure_not_alerted()`) actually correct?**
  _`scan_logger_liveness()` has 14 INFERRED edges - model-reasoned connections that need verification._